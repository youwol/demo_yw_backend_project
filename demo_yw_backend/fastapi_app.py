...
from demo_yw_backend.auto_generated import version
from demo_yw_backend.dependencies import dependencies
import cowsay

app: FastAPI = FastAPI(
    title="my backend",
    root_path=f"http://localhost:{dependencies().yw_port}/backends/demo_yw_backend/{version}",
)
...
class CowSayBody(BaseModel):
    message: str
    character: Literal['beavis', 'cheese', 'cow', 'daemon', 'dragon', 'fox', 'ghostbusters', 'kitty',
    ...
    async with ContextFactory.proxied_backend_context(request).start(
        ...
        asset = await files_client.upload(
            data=aiohttp_file_form(
                filename="demo_yw_backend.uploaded.txt",
...
class CustomAssetBody(BaseModel):
    ...
    id: Optional[str] = "demo_yw_backend-custom-foo-id"
    tags: Optional[List[str]] = ["demo_yw_backend", "create-asset"]
