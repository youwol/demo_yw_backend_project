...
from demo_yw_backend import __version__
from demo_yw_backend.environment import Configuration, Environment
from demo_yw_backend.router import router as root_router
...
def create_app(configuration: Configuration) -> FastAPI:
    ...
    app: FastAPI = FastAPI(
        title="demo_yw_backend",
        # Root path is always served from localhost (using the py-youwol server).
        root_path=f"{root_base}:{configuration.yw_port}/backends/demo_yw_backend/{__version__}",
        lifespan=lifespan,
    )
